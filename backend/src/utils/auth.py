...
def extract_groups_from_claims(claims: dict) -> List[str]:
    ...
    if cognito_groups:
        if isinstance(cognito_groups, str):
            groups.extend([g.strip() for g in cognito_groups.split(',') if g.strip()])
    ...
    if custom_groups:
        if isinstance(custom_groups, str):
            groups.extend([g.strip() for g in custom_groups.split(',') if g.strip()])
    ...
    for attr in idp_group_attrs:
        ...
        if idp_groups:
            if isinstance(idp_groups, str):
                groups.extend([g.strip() for g in idp_groups.split(',') if g.strip()])
